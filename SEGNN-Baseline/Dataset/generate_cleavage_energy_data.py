...
ROOT = Path("./cleavage_energy")
ROOT.mkdir(exist_ok=True)
DATA = ROOT.joinpath("raw_data.json")
...
def collect_stats(data: List[dict]):
    ...
    cleavage_energy = np.vstack(cleavage_energy)

    cleavage_energy_mean = np.mean(cleavage_energy, axis=0)
    cleavage_energy_std = np.std(cleavage_energy, axis=0)

    coef_stats = {
        "cleavage_energy_std": cleavage_energy_std.tolist(),
        "cleavage_energy_mean": cleavage_energy_mean.tolist(),
    }
