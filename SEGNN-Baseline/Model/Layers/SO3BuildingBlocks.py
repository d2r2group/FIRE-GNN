...
class O3TensorProductSwishGate(O3TensorProduct):
    """ A bilinear layer, computing CG tensorproduct and passing them through an O3SwishGate.
    ...
    def __init__(self, irreps_in1, irreps_out, irreps_in2=None, tp_rescale=True) -> None:
        ...
        # Build the layers
        super(O3TensorProductSwishGate, self).__init__(irreps_in1, irreps_g, irreps_in2, tp_rescale)
...
class O3SwishGate(torch.nn.Module):
    """
    A gate layer to provide non-linearity. Handles both scalar irreps and larger
    l valued irreps.
    """
    def __init__(self, irreps_g_scalars, irreps_g_gate, irreps_g_gated) -> None:
        super().__init__()
        if irreps_g_gated.num_irreps > 0:
            self.gate = Gate(irreps_g_scalars, [nn.SiLU()], irreps_g_gate, [nn.Sigmoid()], irreps_g_gated)
        else:
            self.gate = nn.SiLU()
