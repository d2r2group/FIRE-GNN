...
train_data = torch.load('./datasets/wfelements_train.pt')
val_data = torch.load('./datasets/wfelements_validation.pt')
test_data = torch.load('./datasets/wfelements_test.pt')
wandb.init(project="equiformer_v2_fine_tune",id="wfelement",resume="allow")
...
stats = np.load("./datasets/wf_element_stats.npz")
...
# Load Checkpoint
checkpoint_path = "./checkpoints/wfelement.pt"
start_epoch = 1
scaler = GradScaler()
...
def train_one_epoch(epoch):
    ...
    for step, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        ...
        with autocast():
            ...
            pred, targ = outputs.detach().cpu(), target.cpu()      
            print("─ batch debug ─")
