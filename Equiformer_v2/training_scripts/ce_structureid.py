...
train_data = torch.load('./datasets/structureid_train.pt')
val_data = torch.load('./datasets/structureid_validation.pt')
test_data = torch.load('./datasets/structureid_test.pt')
wandb.init(project="equiformer_v2_fine_tune",id="cestrucfull",resume="allow")
...
# Load Checkpoint
checkpoint_path = "./checkpoints/structureid1.pt"
start_epoch = 1
scaler = GradScaler()
...
def train_one_epoch(epoch):
    ...
    for step, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        ...
        all_targs.append(target.detach().cpu())
        # Backward pass
    ...
    # Compute metrics
    train_mae  = calculate_mae(all_preds, all_targs)
    train_mape = calculate_mape(all_preds, all_targs)
    ...
    return avg_loss, train_mae, train_mape
