...
train_data = torch.load('./datasets/wfsgnum_train.pt')
val_data = torch.load('./datasets/wfsgnum_validation.pt')
test_data = torch.load('./datasets/wfsgnum_test.pt')
wandb.init(project="equiformer_v2_fine_tune",id="wfsgnum",resume="allow")
...
stats = np.load("./datasets/wf_sgnum_stats.npz")
...
# Load Checkpoint
checkpoint_path = "./checkpoints/wfsgnum.pt"
start_epoch = 1
scaler = GradScaler()
