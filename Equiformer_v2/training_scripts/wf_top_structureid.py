...
train_data = torch.load('./datasets/wf_top_structureid_train.pt')
val_data = torch.load('./datasets/wf_top_structureid_validation.pt')
test_data = torch.load('./datasets/wf_top_structureid_test.pt')
wandb.init(project="equiformer_v2_fine_tune",id = "wftopstruc",resume="allow")
...
stats = np.load("./datasets/wf_top_structureid_stats.npz")      # ← your saved normalization constants
...
# Load Checkpoint
checkpoint_path = "./checkpoints/wf_top_structureid.pt"
...
def validate(epoch):
    ...
    mu_b  = mu.view(1)
    std_b = std.view(1)
    pred_eV = all_predictions * std_b + mu_b
    targ_eV = all_targets     * std_b + mu_b

    # compute human-readable metrics
    val_mae   = torch.mean((pred_eV - targ_eV).abs()).item()
    val_mape  = (torch.mean((pred_eV - targ_eV).abs() / (targ_eV.abs() + 1e-8)) * 100).item()

    wandb.log({
        "val_loss": avg_loss,
        "val_mae":  val_mae,
        "val_mape": val_mape
    }, step=previous_max_step + epoch)
    logger.info(
        f"Validation Loss = {avg_loss:.4f}, MAE = {val_mae:.4f}, MAPE = {val_mape:.2f}%"
    )
...
def test(epoch):
    ...
    test_mae  = torch.mean((pred_eV - targ_eV).abs()).item()
    test_mape = (torch.mean((pred_eV - targ_eV).abs() / (targ_eV.abs() + 1e-8)) * 100).item()
    ...
    logger.info(
        f"Test Loss = {avg_loss:.4f}, MAE = {test_mae:.4f}, MAPE = {test_mape:.2f}%"
    )
    return avg_loss, mae, mape
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'epoch': epoch
        }, './checkpoints/wf_top_structureid.pt')
