...
class EquiformerV2_OC20(BaseModel):
    ...
    def forward(self, data):
        ...
        # Energy estimation
        ###############################################################
        node_energy = self.energy_block(x).embedding 
        node_energy = node_energy.narrow(1, 0, self.num_targets)  # [num_nodes,2,K]
        node_energy = node_energy.mean(dim=2)   
        
        num_graphs = len(data.natoms)
        energy = torch.zeros(num_graphs, self.num_targets, device=node_energy.device, dtype=node_energy.dtype)
