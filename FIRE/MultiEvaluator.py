...
class WFEvaluator(Evaluator):
    ...
    def main(self):
        ...
        #     f"train/WF Bottom MAE": total_wf_top_mae / num_batches,
        #     f"train/WF Bottom RMSE": (total_wf_top_mse / num_batches)**(1/2),
        #     f"train/WF Bottom MAPE": total_wf_top_mape / num_batches
        # })
        ...
        print(f"Dataset size: {len(self.test_loader.dataset)}")

        batch_residual_wf_top = []
        batch_residual_wf_bottom = []
        batch_residual_ce = []
        self.model.eval()
        with torch.inference_mode():
            for data in tqdm(self.test_loader, f"test Batch", leave=False):
                ...
                batch_residual_wf_top.append(l1_loss(wf_top_pred, wf_top_true, reduction='none'))
                batch_residual_wf_bottom.append(l1_loss(wf_bot_pred, wf_bot_true, reduction='none'))
                batch_residual_ce.append(l1_loss(ce_pred, ce_true, reduction='none'))
        ...
        # print({
        #     f"test/WF Top Residuals": batch_residual_wf_top,
        #     f"test/WF Bottom Residuals": batch_residual_wf_bottom,
        #     f"test/CE Residuals": batch_residual_ce
        # })
